...
{   
    ...
    HAL_GPIO_WritePin(htmc->cs_port, htmc->cs_pin, GPIO_PIN_RESET); // Pull CS pin low
    HAL_Delay(10);
    HAL_SPI_Transmit(htmc->hspi, tx_buff, 5, HAL_MAX_DELAY); // Transmit data over SPI
    HAL_Delay(10);
    HAL_GPIO_WritePin(htmc->cs_port, htmc->cs_pin, GPIO_PIN_SET); // Pull CS pin high
}
...
uint32_t TMC5160_SPI_Read(TMC5160_TypeDef *htmc, uint8_t reg_address)
{
    ...
    uint32_t data = (uint32_t) rx_buff[1] << 24 | (uint32_t) rx_buff[2] << 16 | (uint32_t) rx_buff[3] << 8 | (uint32_t) rx_buff[4] << 0;
